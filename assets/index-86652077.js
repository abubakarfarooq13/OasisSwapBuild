...
`;function ht({borderColor:e,type:i,...r}){return t.jsx(ft,{autoComplete:"off",autoCorrect:"off",autoCapitalize:"off",spellCheck:"false",...r,$isPassword:i==="password",type:i==="password"?"text":i,$borderColor:e})}const mt=({value:e="",numInputs:i=6,onChange:r,renderInput:o,shouldAutoFocus:a=!1,inputType:u="number",containerStyle:y,inputStyle:c,onFocus:h,onBlur:E})=>{const[p,m]=L.useState(0),I=L.useRef([]),v=()=>e?e.toString().split(""):[],W=u==="number";L.useEffect(()=>{I.current=I.current.slice(0,i)},[i]),L.useEffect(()=>{var s;a&&((s=I.current[0])==null||s.focus())},[a]);const B=s=>(W?!isNaN(Number(s)):typeof s=="string")&&s.trim().length>=1,H=s=>{const{value:l}=s.target;if(B(l)){const x=v(),[O,C]=l.split(""),M=x[p]===O?C:O;_(M),T(p+1)}else{const x=s.nativeEvent;x.data===null&&x.inputType==="deleteContentBackward"&&(s.preventDefault(),_(""),T(p-1))}},U=()=>s=>{h==null||h(),m(s)},R=()=>{E==null||E(),m(p-1)},A=s=>{const l=v();[s.code,s.key].includes("Backspace")?(s.preventDefault(),_(""),l[p]||T(p-1)):s.code==="Delete"?(s.preventDefault(),_("")):s.code==="ArrowLeft"?(s.preventDefault(),T(p-1)):s.code==="ArrowRight"||s.key===l[p]?(s.preventDefault(),T(p+1)):(s.code==="Spacebar"||s.code==="Space"||s.code==="ArrowUp"||s.code==="ArrowDown"||W&&isNaN(+s.key)&&!((s.metaKey||s.ctrlKey)&&s.key==="v"))&&s.preventDefault()},T=s=>{var x;const l=Math.max(Math.min(i-1,s),0);I.current[l]&&((x=I.current[l])==null||x.focus(),m(l))},_=s=>{const l=v();l[p]=s[0],P(l)},P=s=>{const l=s.join("");r(l)},Z=(s,l)=>{s.preventDefault();const x=v();let O=l;const C=s.clipboardData.getData("text/plain").split("").slice(0,i);if(W&&C.some(g=>isNaN(Number(g))))return;let M=0;for(let g=0;g<i;++g)g>=l&&(x[g]=C[M++]??"",x[g]&&O++),x[g]||(x[g]="");T(O),P(x)};return t.jsx("div",{style:{display:"flex",alignItems:"center",...y},children:Array.from({length:i},(s,l)=>l).map(s=>t.jsx(L.Fragment,{children:o({value:v()[s]??"",ref:l=>I.current[s]=l,onChange:H,onFocus:()=>U()(s),onBlur:R,onKeyDown:A,onPaste:l=>Z(l,s),autoComplete:"off",maxLength:1,"aria-label":`Please enter OTP character ${s+1}`,style:{textAlign:"center",...c},type:u},s)},s))})},bt=f.div`
  display: flex;
...
`,gt=f.input`
  ...
  ${({theme:e})=>e.mediaWidth.upToSmall`
    ...
    height: 60px;
  `}
`,jt=e=>{const i=e/60|0,r=o=>o<10?"0"+o:o;return`${r(i)}:${r(e-i*60)}`},Oe=5,yt=Oe*Ae.ONE_MIN;var Ce;(function(e){e.VALIDATE_ERROR="VALIDATE_ERROR",e.SEND_EMAIL_ERROR="SEND_EMAIL_ERROR",e.RATE_LIMIT="RATE_LIMIT"})(Ce||(Ce={}));function Ct({isOpen:e,onDismiss:i,onVerifySuccess:r,email:o,showVerifySuccess:a,verifySuccessTitle:u,verifySuccessContent:y,sendCodeFn:c,verifyCodeFn:h,getErrorMsgFn:E,refreshProfile:p=!0}){const m=ue(),[I,v]=d.useState(""),[W]=De(),[B]=Le(),[H,U]=d.useState(!1),[R,A]=d.useState(),T=ke(),[_,P]=d.useState(!1),Z=Pe("(max-height: 450px)"),[s,l]=d.useState(0),x=R==="SEND_EMAIL_ERROR",O=R==="VALIDATE_ERROR",C=R==="RATE_LIMIT",M=!x&&!C&&s<(Oe-1)*Ae.ONE_MIN,g=d.useRef();d.useEffect(()=>(g.current=setInterval(()=>{l(s-1)},1e3),()=>g.current&&clearInterval(g.current)),[s]);const X=d.useCallback((S=!0)=>{U(!0),S&&T({title:w._({id:"nKExoO"}),summary:w._({id:"vH6fr9"}),type:te.SUCCESS})},[T]),G=d.useCallback(async()=>{if(g.current&&clearInterval(g.current),!!o)try{await(c?c({email:o}):B({email:o}).unwrap()),l(yt),A(void 0)}catch(S){l(0),A(S!=null&&S.status?"SEND_EMAIL_ERROR":"RATE_LIMIT")}},[o,B,c]),$=d.useRef(!1),F=d.useCallback(()=>{$.current||($.current=!0,G())},[G]);d.useEffect(()=>{e?a?X(!1):F():(setTimeout(()=>{A(void 0),v(""),U(!1)},300),$.current=!1)},[e,X,a,F]);const se=$e(),[q,K]=d.useState(!1),re=async()=>{if(!(!o||q))try{K(!0),await(h?h({code:I,email:o}):W({code:I,email:o}).unwrap()),await(r?r():i()),p&&(await se(),X())}catch(S){A("VALIDATE_ERROR"),T({title:w._({id:"SlfejT"}),summary:E==null?void 0:E(S),type:te.ERROR})}finally{K(!1)}},ae=S=>{O&&A(void 0),v(S)},Y=t.jsxs(Ve,{children:[t.jsx(N,{color:m.text,fontWeight:"500",fontSize:"20",children:H?u:t.jsx(j,{id:"VGfNFQ"})}),t.jsx(ze,{color:m.text,cursor:"pointer",onClick:i})]}),J=!x&&!C&&s>0;return t.jsx(We,{isOpen:e,onDismiss:i,minHeight:!1,maxWidth:450,maxHeight:_&&ne?window.innerHeight+"px":void 0,height:Be?He&&Z?"100%":_&&ne?window.innerHeight+"px":void 0:void 0,children:t.jsx(bt,{children:H?t.jsxs(je,{children:[Y,y]}):t.jsxs(je,{children:[Y,t.jsx(ye,{style:{color:x||C?m.red:m.subText},children:C?t.jsx(j,{id:"CJhs9o"}):x?t.jsx(j,{id:"IA1HMj",values:{email:o},components:{0:t.jsx(N,{as:"span",color:m.text})}}):t.jsx(j,{id:"DTZF0k",values:{email:o},components:{0:t.jsx(N,{as:"span",color:m.text})}})}),t.jsx(mt,{containerStyle:{justifyContent:"space-between"},value:I,onChange:ae,numInputs:6,renderInput:S=>t.jsx(gt,{...S,hasError:O,placeholder:"-",type:"number",onFocus:()=>{ne&&P(!0),setTimeout(()=>{window.scrollTo({top:0})},100)},onBlur:()=>ne&&P(!1)})}),(J||M)&&t.jsxs(ye,{style:{width:"100%",textAlign:"center"},children:[J&&t.jsx(j,{id:"RSzSFn",values:{0:jt(s),1:M?".":""}})," ",M&&t.jsx(j,{id:"U9/AEt",components:{0:t.jsx(N,{as:"span",color:m.primary,style:{cursor:"pointer"},onClick:G})}})]}),x?t.jsx(ce,{height:"36px",onClick:G,children:t.jsx(j,{id:"MyjAbr"})}):t.jsx(ce,{height:"36px",disabled:I.length<6||C||q,onClick:re,children:q?t.jsx(Ge,{children:t.jsx(j,{id:"bl9677"})}):t.jsx(j,{id:"uSMfoN"})})]})})})}const St=f(Fe)`
  position: absolute;
  right: 13px;
  top: 0;
  bottom: 0;
